...
const LatestNews: React.FC<ILatestNewsProps> = (props: ILatestNewsProps) => {
    ...
    const [currentNewsNews, setCurrentNewsNews] = React.useState<string | null>(null);
    const [commentText, setCommentText] = React.useState<string>(''); // Ajout du state pour le texte du commentaire
    ...
    const handleToggleCommentPart = (newsNews: string | null = null) => { // Modifier la signature de la fonction
        setShowCommentPart(!showCommentPart);
        setCurrentNewsNews(newsNews); // Utiliser setCurrentNewsNews au lieu de setCurrentNewsId
    };
    ...
    const handleSubmitComment = async () => {
        try {
            ...
            if (currentNewsNews !== null) { // Utiliser currentNewsNews au lieu de currentNewsId
                await commentService.postComment(commentText, currentNewsNews); // Utiliser currentNewsNews au lieu de currentNewsId
                alert('Comment submitted successfully!');
                setShowCommentPart(false);
                setCommentText(''); // Efface le texte du commentaire après la soumission

            } else {
                alert('No news ID selected.');
            }
        } catch (error) {
                                        ...
                                        <div className={styles.card_button}>
                                            ...
                                            <div className={styles.CB_reactions}>
                                                ...
                                                <div style={{ paddingRight: '5px' }}>
                                                    <button className={styles.CB_reactions_BTN} onClick={() => handleToggleCommentPart(item.News)}>
